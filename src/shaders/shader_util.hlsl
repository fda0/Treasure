...
static Mat3 Mat3_FromMat4(Mat4 mat)
{
  Mat3 res;
  res._m00 = mat._m00;
  res._m01 = mat._m01;
...
static Quat Quat_FromNormalizedPair(float3 a, float3 b)
{
  float3 crs = cross(a, b);
  Quat res =
  {
    crs.x,
    crs.y,
...
static Quat Quat_FromZupCrossV3(float3 a)
{
  return Quat_FromNormalizedPair(float3(0,0,1), normalize(a));
}
...
static float RoundedRectSDF(float2 pos, float2 center, float2 half_dim, float r)
{
  float2 d2 = (abs(pos - center) - half_dim + float2(r,r));
  float l = length(max(d2, float2(0.f,0.f)));
  return min(max(d2.x, d2.y), 0.f) + l - r;
}

static float HexagonSDF(float2 pos, float r)
{
...
static float HexagonBoardSDF(float2 position, float scale)
{
  position /= scale;
